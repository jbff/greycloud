...
class TestCreateClient:
    ...
    def test_create_client_oauth_auto_reauth(self, mock_auth):
        ...
        def check_output_side_effect(*args, **kwargs):
            call_count[0] += 1
            if call_count[0] == 1:
                error = subprocess.CalledProcessError(1, "gcloud", stderr=b"application-default")
                raise error
            return "new-token"
        
        with patch('greycloud.auth.subprocess.check_output', side_effect=check_output_side_effect):
            with patch('greycloud.auth.subprocess.run', return_value=MagicMock(returncode=0)):
                with patch('greycloud.auth.genai') as mock_genai:
                    mock_client = MagicMock()
                    mock_genai.Client.return_value = mock_client
